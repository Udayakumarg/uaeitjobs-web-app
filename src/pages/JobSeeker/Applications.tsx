...
export default function JobSeekerApplications() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      {!page ? <div className="mt-6 h-64 animate-pulse rounded-lg bg-slate-200" /> : page.content.length === 0 ? <EmptyState title="No applications yet." /> : (
          ...
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody>{page.content.map((application) => (
              <tr key={application.id} className="border-t border-slate-100">
                <td className="p-4 font-semibold text-slate-950">{application.job ? <Link className="text-blue-700" to={`/jobs/${application.job.id}`}>{application.job.title}</Link> : 'Job'}</td>
