...
export default function SavedJobs() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      {!jobs ? <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3" aria-live="polite" aria-busy="true"><span className="sr-only">Loading saved jobs</span>{Array.from({ length: 3 }).map((_, index) => <CardSkeleton key={index} />)}</div> : null}
